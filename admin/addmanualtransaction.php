...
if (empty($uid)) {
    redirect_header('transactions.php', 5, "Could not find user '" . $uname . "'.");
}

if (empty($expirationdate) || empty($amount)) {
    redirect_header('transactions.php', 5, 'You must include both the amount and the expiration date.');
}
...
$sql    = 'select st.groupid from ' . $xoopsDB->prefix('subscription_type') . ' st, ' . $xoopsDB->prefix('subscription') . ' s ' . " where s.subtypeid = st.subtypeid and s.subid = $subid";
$result = $xoopsDB->query($sql);
...
$sql = 'insert into ' . $xoopsDB->prefix('subscription_user') . ' (subid, uid, expiration_date, intervaltype, intervalamount, ' . "amount, cancel) values ($subid, $uid, '$expDate', 'x', 0, '$amount','N')";
