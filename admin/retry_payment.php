...
if (!$gw->isDirect()) {
    redirect_header('tx_detail.php?txid=' . $txid, 2, 'You must use a direct payment gateway for this function to work.');
}
$sql = 'select uid from ' . $xoopsDB->prefix('subscription_transaction') . " where id = $txid";
...
if ($paymentResponse->responseCode == 0) {
    ...
    redirect_header("tx_detail.php?txid=$id", 2, 'This transaction failed with the message: ' . $paymentResponse->responseMessage);
}
